...
| `init <agent>`              | Install skills + MCP server config for an AI agent      | `flutter-mcp-toolkit init Codex`                                         |
...
Targets: `Codex` | `cursor` | `codex` | `cline` | `agents-skills` | `all`.

```bash
flutter-mcp-toolkit init Codex        # install for Codex (project-scoped)
